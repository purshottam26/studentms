...
$total_fee = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(amount) as t FROM fees WHERE student_id='$sid'"))['t'] ?? 0;
$paid_fee = mysqli_fetch_assoc(mysqli_query($conn, "SELECT SUM(amount) as t FROM fees WHERE student_id='$sid' AND status='paid'"))['t'] ?? 0;
    ...
    <div class="content">
        <div class="topbar">
            ...
            <div class="topbar-right">
                <a href="student_profile_edit.php" style="padding:9px 16px;background:#4f46e5;color:white;border-radius:8px;text-decoration:none;font-size:13px;font-weight:700;">✏️ Edit Profile</a>
        ...
        <div class="profile-hero">
            ...
            <?php endif; ?>
            <div style="flex:1;">
                <h2 style="font-size:22px;font-weight:800;margin-bottom:6px;"><?php echo htmlspecialchars($student['name']); ?></h2>
                <p style="font-size:13px;opacity:0.85;margin-bottom:3px;">🪪 <?php echo htmlspecialchars($student['student_id']); ?></p>
                <p style="font-size:13px;opacity:0.85;margin-bottom:3px;">📚 <?php echo htmlspecialchars($student['course']); ?></p>
                <?php if(!empty($student['father_name'])): ?>
                <p style="font-size:13px;opacity:0.85;">👨 <?php echo htmlspecialchars($student['father_name']); ?></p>
                <?php endif; ?>
            </div>
            <div style="text-align:right;">
                <div style="background:rgba(255,255,255,0.2);padding:10px 16px;border-radius:10px;margin-bottom:8px;">
                    <div style="font-size:11px;opacity:0.8;">Attendance</div>
                    <div style="font-size:22px;font-weight:800;"><?php echo $att_percent; ?>%</div>
                </div>
                <div style="background:rgba(255,255,255,0.2);padding:10px 16px;border-radius:10px;">
                    <div style="font-size:11px;opacity:0.8;">Fee Pending</div>
                    <div style="font-size:18px;font-weight:800;">₹<?php echo number_format($unpaid_fee, 0); ?></div>
                </div>
            </div>
        </div>
        ...
        <div class="info-grid">
            <div class="info-box">
                <label>📧 Email</label>
                <span style="font-size:13px;"><?php echo htmlspecialchars($student['email']); ?></span>
            </div>
            <div class="info-box">
                <label>📱 Mobile</label>
            ...
            <div class="info-box">
                ...
                <span><?php echo !empty($student['whatsapp']) ? htmlspecialchars($student['whatsapp']) : '—'; ?></span>
            </div>
            <div class="info-box">
                <label>🎂 Date of Birth</label>
                <span><?php echo !empty($student['dob']) ? date('d M Y', strtotime($student['dob'])) : '—'; ?></span>
            </div>
            <div class="info-box">
            ...
            <div class="info-box">
                ...
                <span><?php echo htmlspecialchars($student['pincode']); ?></span>
            </div>
            <div class="info-box" style="grid-column:span 3;">
                <label>🪪 Aadhaar Number</label>
                <span><?php echo htmlspecialchars($student['aadhaar']); ?></span>
            </div>
        </div>
        ...
        <div class="box">
            ...
            <div style="font-size:12px;color:#64748b;margin-top:6px;">
                <?php if($att_percent >= 75): ?>✅ Attendance theek hai!
                <?php elseif($att_percent >= 50): ?>⚠️ Attendance improve karo!
                <?php else: ?>❌ Attendance bahut kam hai!
                <?php endif; ?>
            </div>
        </div>
        <?php endif; ?>
        ...
        <div class="box">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:16px;">
                <div style="background:#f8fafc;border-radius:10px;padding:14px;text-align:center;">
                    ...
                    <div style="font-size:20px;font-weight:800;">₹<?php echo number_format($total_fee,0); ?></div>
                </div>
...
window.onload = function(){
    ...
    if(bar) setTimeout(() => { bar.style.width = '<?php echo $att_percent; ?>%'; }, 300);
}
</script>
